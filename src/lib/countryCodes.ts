// Static, bundled country + dial-code list -- generated from the
// 'world-countries' npm package (ISO 3166-1 / ITU-T calling codes).
// Intentionally NOT fetched from the database: dial codes are static
// reference data, and every earlier bug in PhoneInput (RLS gaps,
// session-restoration races, wrong Supabase project, deploy lag,
// cache poisoning) came from treating them as if they weren't. This
// list ships in the JS bundle itself, so the dial-code dropdown can
// never depend on a network call, a database, or a session succeeding.
export type CountryCode = { name: string; iso2: string; dial: string };
...
export const COUNTRY_CODES: CountryCode[] = [
  ...
  { name: 'Vatican City', iso2: 'VA', dial: '+3' },
